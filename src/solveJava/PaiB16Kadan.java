...
public class PaiB16Kadan {
	...
	private static int nextFlowerCount(String[][] parkMaps, int hCount, int wCount) {
		...
		if (hCount > 0) {
			if (parkMaps[hCount - 1][wCount].equals("#")) {
				nextFlowerCount += 1;
			}
		}
		if(wCount > 0) {
			if (parkMaps[hCount][wCount - 1].equals("#")) {
				nextFlowerCount += 1;
			}
		}
		if(hCount < parkMaps.length - 1) {
			if (parkMaps[hCount + 1][wCount].equals("#")) {
				nextFlowerCount += 1;
			}
		}
		if(wCount < parkMaps[hCount].length - 1) {
			if (parkMaps[hCount][wCount + 1].equals("#")) {
				nextFlowerCount += 1;
			}
		}
	...
	private static boolean isFlower(String[][] parkMaps, int hCount, int wCount) {
		if (parkMaps[hCount][wCount].equals("#")) {
			return true;
		} else {
			return false;
		}
